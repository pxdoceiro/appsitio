  ...
  try {
    ...
    if (!ENV.sttApiUrl) {
      return {
        ...
        code: "SERVICE_ERROR",
        details: "STT_API_URL is not set",
      };
    }
    if (!ENV.sttApiKey) {
      return {
        ...
        code: "SERVICE_ERROR",
        details: "STT_API_KEY is not set",
      };
    }

    // Step 2: Load audio (URL or data URL)
    let audioBuffer: Buffer;
    let mimeType: string;
    try {
      if (options.audioUrl.startsWith("data:")) {
        const parsed = parseDataUrl(options.audioUrl);
        if (!parsed) {
          return {
    ...
    const baseUrl = ENV.sttApiUrl.endsWith("/") ? ENV.sttApiUrl : `${ENV.sttApiUrl}/`;
